...
func (d *Downloader) Download(ctx context.Context, region string) (string, error) {
	...
	// Verify SHA256 — a confirmed mismatch is always fatal, but a missing
	// entry only warns.  This matches finfocus-core's lenient policy and
	// avoids hard failures when checksums.txt is incomplete or was generated
	// with path prefixes.  See: https://github.com/rshade/finfocus-plugin-aws-public/issues/351
	expectedHash, ok := d.checksums[archiveName]
	if !ok {
		d.logger.Warn().
			Str("file", archiveName).
			Msg("no checksum entry found, skipping verification")
	} else if err := d.verify(tarballPath, expectedHash); err != nil {
...
// Leading "./" prefixes on filenames are stripped for compatibility with
// checksums generated by "sha256sum ./*.tar.gz" (which preserves the "./" path).
